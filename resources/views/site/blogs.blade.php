<x-site.layout title="Daniox - blogs">
    <div class=" py-8">
        <h2 class="text-4xl font-bold text-center mb-8">Related Blog Posts</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 px-4 sm:px-8 lg:px-16">
            @forelse ($blogs as $item)
                <article class="bg-gray-800 rounded-lg shadow-lg overflow-hidden">
                    <a href="{{ route('site.blog',$item->id) }}" target="_blank">
                        <img src="{{ $item->cover_image }}" class="w-full h-40 object-cover" alt="Image 1">
                    </a>
                    <div class="p-4">
