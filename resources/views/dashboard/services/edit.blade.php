<x-dashboard.layout.default title="Edit Service">
    ...
    <x-ui.card heading="Edit Service">
                ...
                <div class="grid grid-cols-2 space-y-5">
                    <div class="px-2 mt-5">
                        <x-ui.form.input id="serv-name" name="name" label="Service Name" placeholder="Enter Name"
                            class="w-full p-2 text-lg" value="{{$service->name}}" required />
                    </div>
                    <div class="px-2">
                        <x-ui.form.simple-select id="serv-tech" name="technology_id" label="Technology" required>
                            @foreach ($technologies as $tech)
                                <option value="{{$tech->id}}" {{$tech->id == $service->technology_id ? 'selected' : ''}}>{{$tech->name}}</option>
                            @endforeach
                    ...
                    <div class="px-2">
                        <x-ui.form.input id="email" name="email" label="Email"
                            class="w-full p-2 text-lg" value="{{$service->email}}" required />
                    </div>
                    <div class="px-2">
                        <x-ui.form.input id="phone" name="phone" label="Phone"
                            class="w-full p-2 text-lg" value="{{$service->phone}}" />
                    </div>

                    <div class="px-2">
                        <x-ui.form.image-upload name="cover_image" oldData="{{$service->cover_image ? asset($service->cover_image) : ''}}" />
                    </div>
