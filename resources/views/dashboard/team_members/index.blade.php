<x-dashboard.layout.default title="Team Members">
    ...
    <x-ui.card class="mx-auto">
        ...
        <div class="relative overflow-x-auto sm:rounded-lg py-3 px-2">
            <table id="team-members" class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-white datatable">
                ...
                <tbody>
                    @foreach ($team_members as $item)
                        <tr
                            ...
                            <th scope="row"
                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ $item->member_type == 1 ? 'Executive' : 'Research' }}
                            </th>
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="image-placeholder" data-src="{{ asset($item->image) }}"></div>

                            </td>
                            <td class="px-6 py-4">
                                @if (count($item->social) > 0)

                                    @foreach ($item->social as $social)
                                        <a href="{{ $social['url'] }}"
                                            class="space-y-2 bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300"
