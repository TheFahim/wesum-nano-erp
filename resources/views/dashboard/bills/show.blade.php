                            ...
                            <header class="flex justify-start items-start my-8">
                                <div class="text-left">
                                    ...
                                    <div class="grid grid-cols-2 text-sm">
                                        ...
                                        <div class="border border-gray-400 p-1 bg-blue-100 font-semibold">{{ $bill->bill_no ?? '-' }}</div>
                                        <div class="font-bold p-1 text-left">CUSTOMER ID</div>
                                        <div class="border border-gray-400 p-1">{{ $bill->challan->quotation->customer->customer_no }}</div>
                                        ...
                                        <div class="border border-gray-400 p-1">{{ $bill->challan->delivery_date ? \Carbon\Carbon::parse($bill->challan->delivery_date)->format('d/m/Y') : 'N/A' }}</div>
                                    </div>
                                </div>
                            </header>
                            <div class="h-8"></div>
                            ...
                            <section class="mb-8 text-sm">
                                <div class="flex justify-between items-stretch gap-x-8">
                                    ...
                                    <div class="w-1/2 flex flex-col">
                                        ...
                                        <div class="border border-gray-400 p-3 space-y-1 break-words flex-grow">
                                            <p><strong class="w-24 inline-block">Company</strong> : {{ $bill->challan->quotation->customer->company_name }}</p>
                                            <p><strong class="w-24 inline-block">Address</strong> : {{ $bill->challan->quotation->customer->address }}</p>
                                            <p><strong class="w-24 inline-block">Phone</strong> : {{ $bill->challan->quotation->customer->phone }}</p>
                                            <p>&nbsp;</p>
                                            <p>&nbsp;</p>
                                        </div>
                                    </div>
                                ...
                                <table id="products-table" class="w-full border-collapse text-sm">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="bg-blue-900 text-white p-2 border border-gray-500 text-right">PRICE</th>
                                            <th class="bg-blue-900 text-white p-2 border border-gray-500 text-right w-24">AMOUNT</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($bill->challan->quotation->products as $product)
                                            <tr>
                                                ...
                                                <td class="border border-gray-400 p-2 text-right align-top">{{ number_format($product->price, 2) }}</td>
                                                <td class="border border-gray-400 p-2 text-right align-top font-bold">{{ number_format($product->amount, 2) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
