    ...
    <x-dashboard.ui.bread-crumb>
        <li class="inline-flex items-center">
            <a href="{{ route('users.index') }}"
                ...
                <x-ui.svg.users class="h-4 w-4 me-2" />
                &nbsp; Users
            </a>
        </li>
                        ...
                        <div>
                            ...
                            <ul
                                ...
                                @foreach ($techs as $item)
                                    <li class="w-full border-b border-gray-200 rounded-t-lg dark:border-gray-600">
                                        <div class="flex items-center ps-3">
                                            <input id="tech-{{$item->id}}" type="checkbox" name="technologies[]" value="{{$item->id}}"
                                                {{in_array($item->id, $userTechs) ? 'checked' : ''}}
                                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                                            <label for="tech-{{$item->id}}"
                                                class="w-full py-3 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{$item->name}}</label>
                                        </div>
                                    </li>
                                @endforeach
